...
graph TD
    ...
    classDef juizes fill:#561818,stroke:#d4af37,stroke-width:2px,color:#fff;
    classDef reis fill:#0d2b45,stroke:#5c80bc,stroke-width:2px,color:#fff;
    ...
    subgraph Era da Formação [Sangue e Bronze]
        V1 --> V2(Vol II: A Praga do Nilo)
        V2 --> V3(Vol III: O Vale dos Cântaros)
        V3 --> V4(Vol IV: O Deus Decapitado)
    end

    subgraph Era dos Reis e Impérios [A Máquina de Guerra]
        V4 --> V5(Vol V: A Coroa de Cinzas)
        V5 --> V6(Vol VI: O Fogo do Carmelo)
        V6 --> V7(Vol VII: O Cerco de Jerusalém)
        V7 --> V8(Vol VIII: A Boca da Besta)
    end

    subgraph A Batalha Final [A Morte da Morte]
        V8 --> V9(Vol IX: O Sangue do Cordeiro)
        V9 --> V10(Vol X: O Trono Branco)
    end

    %% Classes
    class P,V0,V1 primordial;
    class V2,V3,V4 juizes;
    class V5,V6,V7,V8 reis;
    class V9,V10 final;
    
    %% Indicador de Status Atual
    style V7 fill:#d4af37,color:#000,stroke:#fff,stroke-width:4px,stroke-dasharray: 5 5;
```
